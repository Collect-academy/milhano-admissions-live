...
export default async function DashboardPage({
  ...
  const legacyCutoff = "2026-09-07";
  const range = requestedRange.start > legacyCutoff
    ? { ...requestedRange, key: "custom" as const, start: legacyCutoff, end: legacyCutoff, label: "7 Sep 2026 · Legacy cutoff" }
    : { ...requestedRange, end: requestedRange.end > legacyCutoff ? legacyCutoff : requestedRange.end };
  ...
  return (
    <DashboardLayout
      eyebrow={tr(locale, "Milhano · Admissions V1 Legacy", "Milhano · Admisiones V1 Legacy")}
      statusLabel={`${tr(locale, "Period", "Periodo")} ${dateLabel(
        range.start,
      )} – ${dateLabel(range.end)}`}
      subtitle={tr(locale, "Historical dashboard preserved through September 7, 2026.", "Dashboard histórico preservado hasta el 7 de septiembre de 2026.")}
      title={tr(locale, "Admissions Summary · Legacy", "Resumen de Admisiones · Legacy")}
    >
      <AdmissionsVersionSwitcher current="legacy" />

      <div className="v2-cutover-note legacy-cutover-note"><strong>V1 congelada al 7 Sep 2026.</strong><span>La operación nueva se consulta en <Link href="/">V2 · Actual</Link>.</span></div>
