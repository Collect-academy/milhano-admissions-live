...
-- Pipeline IDs confirmed directly from GHL on 2026-09-16:
-- Legacy: Antiguo Leads Milhano      = L9nSVkwjFsHN5WFDs3Aa
-- Setter: Leads Milhano (Setter pipeline) = GYqHbZyWUxxc3K03efVT
-- Closer: Leads Milhano (Closer Pipeline) = z1FEJfbtOHusjdwe40Ko
-- No auxiliary pipeline registry is required.
...
pipeline_meta as (
  select
    'L9nSVkwjFsHN5WFDs3Aa'::text as legacy_pipeline_id,
    'GYqHbZyWUxxc3K03efVT'::text as setter_pipeline_id,
    'z1FEJfbtOHusjdwe40Ko'::text as closer_pipeline_id,
    'Antiguo Leads Milhano'::text as legacy_pipeline_name,
    'Leads Milhano (Setter pipeline)'::text as setter_pipeline_name,
    'Leads Milhano (Closer Pipeline)'::text as closer_pipeline_name
)
select jsonb_build_object(
  'meta',jsonb_build_object(
    ...
    'legacy_pipeline_name',(select legacy_pipeline_name from pipeline_meta),
    'setter_pipeline_id',(select setter_pipeline_id from pipeline_meta),
    'setter_pipeline_name',(select setter_pipeline_name from pipeline_meta),
    'closer_pipeline_id',(select closer_pipeline_id from pipeline_meta),
    'closer_pipeline_name',(select closer_pipeline_name from pipeline_meta),
